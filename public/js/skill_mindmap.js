...
var m = [20, 100, 20, 100],
      ...
      var selectNode = function(target){
        if(target){
          var sel = d3.selectAll('#skillmap svg .node').filter(function(d){return d.id==target.id})[0][0];
          if(sel){
            select(sel);
          }
        }
...
*/
      ...
      loadJSON('/public/data_factory/skill_mindmap.json');
